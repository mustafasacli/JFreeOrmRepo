 ...
 */
public abstract class ExternalBaseDL implements IBaseDL {
     ...
     * @throws Exception
     */
    protected ExternalBaseDL() throws Exception {
        this("external");
    }
     ...
     */
    protected ExternalBaseDL(String connectionName) throws Exception {

        try {
            conf = ConfigurationManager.getConfiguration(connectionName);
            dt = ConfigurationManager.GetDriverType(connectionName);

            if (dt != DriverType.External) {
                throw new Exception("Driver Type must be external.");
            }
    ...
    protected ExternalBaseDL(String driver, String url) throws Exception {
    ...
    protected ExternalBaseDL(String driver, String url, String user, String pass)
            throws Exception {
        try {
            conf = new DbConfiguration();
            conf.setDriver(driver);
    ...
    private final void init_connection() throws Exception {
        ...
        if (StringUtil.isValidString(conf.getUser())) {
            db_conn = new ExternalDbConnection(conf.getDriver(), conf.getUrl(),
